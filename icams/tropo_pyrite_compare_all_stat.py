...
def cmdLineParse():
    ...
    parser.add_argument('MinCoh',type=float, default=0.15, help='Coherence threshold for calculating the statistic results.')
...
def main(argv):
    ...
    inps = cmdLineParse()
    root_path = os.getcwd()
    work_dir = root_path + '/PyRite_Compare'

    file_list = glob.glob(work_dir + '/geo_*compare.h5')
    text_file = 'pyrite_compare_stat.txt'
    
    if os.path.isfile(text_file):
        os.remove(text_file)
    
    N = len(file_list)
    STR0 = '  Pair ' + ' OrgStd  ZenithLinearStd LosLinearStd LosKrigingStd ZenithLinearMean OrgMean LosLinearMean LosKrigingMean'
    call_str = 'echo ' + STR0 + ' > ' + text_file
    os.system(call_str)
    
    for i in range(N):
        file0 = file_list[i]
        name0 = os.path.basename(file0)
        pair = name0.split('_')[1]
        coh,atr = ut.read_hdf5(file0,datasetName='coherence')
        where_are_NaNs = np.isnan(coh)
        coh[where_are_NaNs] = 0
        
        
        insar0,atr = ut.read_hdf5(file0,datasetName='insar')
        where_are_NaNs = np.isnan(insar0)
        insar0[where_are_NaNs] = 0
        
        los_kriging,atr = ut.read_hdf5(file0,datasetName='los_kriging_cor')
        where_are_NaNs = np.isnan(los_kriging)
        los_kriging[where_are_NaNs] = 0
        
        los_linear,atr = ut.read_hdf5(file0,datasetName='los_linear_cor')
        where_are_NaNs = np.isnan(los_linear)
        los_linear[where_are_NaNs] = 0
        
        zenith_linear,atr = ut.read_hdf5(file0,datasetName='zenith_linear_cor')
        where_are_NaNs = np.isnan(zenith_linear)
        zenith_linear[where_are_NaNs] = 0
        
        insar0[coh<inps.MinCoh] = 0
        los_kriging[coh<inps.MinCoh] = 0
        zenith_linear[coh<inps.MinCoh] = 0
        los_linear[coh<inps.MinCoh] = 0
        
        std_insar0 = np.std(insar0[insar0!=0])
        mean_insar0 = np.mean(insar0[insar0!=0])
        
        std_lk0 = np.std(los_kriging[los_kriging!=0])
        mean_lk0 = np.mean(los_kriging[los_kriging!=0])
        
        std_ll0 = np.std(los_linear[los_linear!=0])
        mean_ll0 = np.mean(los_linear[los_linear!=0])
        
        std_zl0 = np.std(zenith_linear[zenith_linear!=0])
        mean_zl0 = np.mean(zenith_linear[zenith_linear!=0])
        
        STR = pair + ' ' + str(round(std_insar0*10000)/100) + ' ' + str(round(std_zl0*10000)/100) + ' ' + str(round(std_ll0*10000)/100) + ' ' + str(round(std_lk0*10000)/100) + ' ' + str(round(mean_insar0*10000)/100) + ' ' + str(round(mean_zl0*10000)/100) + ' ' + str(round(mean_ll0*10000)/100) + ' ' + str(round(mean_lk0*10000)/100)
        
        print(STR)
        call_str = 'echo ' + STR + ' >> ' + text_file
        os.system(call_str)
