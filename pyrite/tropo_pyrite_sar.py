...
def cmdLineParse():
    parser = argparse.ArgumentParser(description='Generating high-resolution tropospheric delay map using PyRite.',\
...
INTRODUCTION = '''
##################################################################################
   Copy Right(c): 2020, Yunmeng Cao   @PyRite v1.0
   
   Generate InSAR tropospheric delays using ERA5 based on PyRite.
'''

EXAMPLE = """Example:
  
  tropo_pyrite_sar.py geometryRadar.h5 master.slc.par -o 20190101_pyrite.h5
  tropo_pyrite_sar.py geometryRadar.h5 master.slc.par --date 20180101 -o 20180101_pyrite.h5
  tropo_pyrite_sar.py geometryRadar.h5 master.slc.par --method kriging --project los
  tropo_pyrite_sar.py geometryRadar.h5 master.slc.par --date 19700101 --lalo-resclae 10
  tropo_pyrite_sar.py geometryRadar.h5 master.slc.par --method linear --project zenith
  tropo_pyrite_sar.py geometryRadar.h5 master.slc.par --method linear --project zenith 
...
def main(argv):
    ...
    if inps.project =='los':
        ...
        dwet_intp,ddry_intp, lonvv, latvv, hgtuse = ut.pyrite_griddata_los(lon_intp,lat_intp,hgtlvs,ddrylos,dwetlos,attr, maxdem, mindem, inps.lalo_rescale,inps.method,inps.kriging_points_numb)
        ...
        dwet_intp,ddry_intp, lonvv, latvv, hgtuse = ut.pyrite_griddata_zenith(lonlist,latlist,hgtlvs,ddry_zenith,dwet_zenith, attr, maxdem, mindem, inps.lalo_rescale,inps.method,inps.kriging_points_numb)
