...
##################################################################################
   ...
   Correcting InSAR tropospheric delays using PyRite based on ERA5 reanalysis data.
'''

EXAMPLE = """example:
  
  tropo_pyrite.py timeseries.h5 geometryRadar.h5 SAR.slc.par --lalo-rescale 10 
  tropo_pyrite.py timeseries.h5 geometryRadar.h5 SAR.slc.par --project zenith
  tropo_pyrite.py timeseries.h5 geometryRadar.h5 SAR.slc.par --project los --lalo-rescale 5
  tropo_pyrite.py timeseries.h5 geometryRadar.h5 SAR.slc.par --project los --method linear
  tropo_pyrite.py timeseries.h5 geometryRadar.h5 SAR.slc.par --project los --method kriging
...
def main(argv):
    ...
    for i in range(len(date_generate)):
        ...
        call_str = 'tropo_pyrite_sar.py ' + geo_file + ' ' + slc_par + ' --date ' + date_generate[i] + ' --method ' + inps.method + ' --project ' + inps.project + ' --lalo-rescale ' + str(inps.lalo_rescale) + ' --kriging-points-numb ' + str(inps.kriging_points_numb)
        os.system(call_str)        
    ...
    call_str =  'pyrite_timeseries.py --date-txt ' + txt_list + ' --project ' + inps.project + ' --method ' + inps.method 
    os.system(call_str)
    ...
    call_str = 'diff_pyrite.py ' + inps.ts_file + ' ' + ' timeseries_pyrite_' + inps.project + '_' + inps.method + '.h5' + ' --add  -o timeseries_pyriteCor' + '_' + inps.project + '_' + inps.method + '.h5'
    os.system(call_str)
    print('Done.')
