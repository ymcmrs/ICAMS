...
from mintpy.utils import network as nt
...
EXAMPLE = '''
    Usage: 
            tropo_pyrite_compare_all.py unwrapIfgram.h5 geometryRadar.h5 master.slc.par --max-tb 12
            tropo_pyrite_compare_all.py unwrapIfgram.h5 geometryRadar.h5 master.slc.par --project los 
            tropo_pyrite_compare_all.py unwrapIfgram.h5 geometryRadar.h5 master.slc.par --method kriging
            tropo_pyrite_compare_all.py unwarpIfgram.h5 geometryRadar.h5 master.slc.par --lalo-rescale 10
            tropo_pyrite_compare_all.py unwarpIfgram.h5 geometryRadar.h5 master.slc.par --kriging-points-numb 20
...
def cmdLineParse():
    ...
    parser.add_argument('--max-tb', dest='max_tb', type=int, default=13, help='maximum temporal baseline')
...
def main(argv):
    ...
    btemp_max = inps.max_tb
    
    date12list00 = nt.get_date12_list(unwFile)
    date12list0 = []
    for k0 in date12list00:
        m0,s0 = k0.split('_')
        m1 = m0[2:8]
        s1 = s0[2:8]
        k00 = m1 + '-' + s1
        date12list0.append(k00)
    #print(date12list0)    
    date12list = nt.threshold_temporal_baseline(date12list0, btemp_max, keep_seasonal=True, btemp_min=0.0)
    print(date12list)
    index_list = []
    
    N = len(date12list0)
    for i in range(N):
        if date12list0[i] in date12list:
            index_list.append(i)
    
    nn = len(index_list)
    root_dir = os.getcwd()
    work_dir = root_dir + '/PyRite_Compare'
    ...
    INC,atr0 = ut.read_hdf5(geo_file,datasetName='incidenceAngle')
    ...
    for i in range(nn):
        ...
        pair = date12list[i]
        Mdate = '20' + pair.split('-')[0]
        Sdate = '20' + pair.split('-')[1]
        pair0 = Mdate + '-' + Sdate
        print(pair0)
        ...
        # gacos 
        sar_folder = root_dir + '/gacos/sar'
        Mgacos = sar_folder + '/' + Mdate + '_tzd.h5'
        Sgacos = sar_folder + '/' + Sdate + '_tzd.h5'

        Mzenith_data,atr0 = ut.read_hdf5(Mgacos,datasetName='aps_sar')
        Szenith_data,atr0 = ut.read_hdf5(Sgacos,datasetName='aps_sar')
        Mdlos_data = Mzenith_data / np.cos(INC/180*np.pi)
        Sdlos_data = Szenith_data / np.cos(INC/180*np.pi)

        gacos_aps = Mdlos_data - Sdlos_data
        ...
        gacos_aps = gacos_aps - gacos_aps[int(atr['REF_Y']),int(atr['REF_X'])]
        ...
        atr['UNIT'] = 'm'
        OUT = work_dir + '/' + pair0 + '_pyrite_compare.h5'
        ut.write_h5(datasetDict, OUT, metadata= atr, ref_file=None, compression=None)
        
        call_str = 'geocode.py ' + OUT + ' --outdir ' +  work_dir
        os.system(call_str)
